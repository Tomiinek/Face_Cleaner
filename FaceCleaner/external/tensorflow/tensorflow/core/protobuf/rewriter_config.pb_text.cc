...
void AppendProtoDebugString(
  ...
  if (msg.layout_optimizer() != 0) {
    o->AppendEnumName("layout_optimizer", ::tensorflow::EnumName_RewriterConfig_Toggle(msg.layout_optimizer()));
  }
  ...
  if (msg.dependency_optimization() != 0) {
    o->AppendEnumName("dependency_optimization", ::tensorflow::EnumName_RewriterConfig_Toggle(msg.dependency_optimization()));
  }
  if (msg.loop_optimization() != 0) {
    o->AppendEnumName("loop_optimization", ::tensorflow::EnumName_RewriterConfig_Toggle(msg.loop_optimization()));
  }
...
bool ProtoParseFromScanner(
  ...
  while(true) {
    ...
    if (identifier == "layout_optimizer") {
      ...
      } else if (value == "AGGRESSIVE" || value == "3") {
