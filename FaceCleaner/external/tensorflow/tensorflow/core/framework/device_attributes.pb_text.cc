...
void AppendProtoDebugString(
    ...
    const ::tensorflow::InterconnectLink& msg) {
  o->AppendNumericIfNotZero("device_id", msg.device_id());
  o->AppendStringIfNotEmpty("type", ProtobufStringToString(msg.type()));
  o->AppendNumericIfNotZero("strength", msg.strength());
}
...
void AppendProtoDebugString(
  ...
  for (int i = 0; i < msg.link_size(); ++i) {
    o->OpenNestedMessage("link");
    ::tensorflow::internal::AppendProtoDebugString(o, msg.link(i));
    ...
    if (identifier == "link") {
      ...
      do {
        ...
        if (!::tensorflow::internal::ProtoParseFromScanner(
            scanner, true, open_char == '{', msg->add_link())) return false;
...
void AppendProtoDebugString(
  ...
  o->AppendNumericIfNotZero("numa_node", msg.numa_node());
  if (msg.has_links()) {
    o->OpenNestedMessage("links");
    ::tensorflow::internal::AppendProtoDebugString(o, msg.links());
    o->CloseNestedMessage();
  }
}
...
bool ProtoParseFromScanner(
  ...
  while(true) {
    ...
    if (identifier == "bus_id") {
      ...
      ProtoSpaceAndComments(scanner);
      if (!::tensorflow::internal::ProtoParseFromScanner(
          scanner, true, open_char == '{', msg->mutable_links())) return false;
    }
  }
}
