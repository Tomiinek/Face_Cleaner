...
int parse_settings(int argc, const char* argv[], bf::path exec_base, po::variables_map& vm, int& verbosity, int& threads, int& max_refinement,
	...
	generic.add_options()
		...
		("manual", "Print description of the required configuration file's parameters.");

	// definition of other command line options
	po::options_description config("Configuration options");
	config.add_options()
		("verbose,v", po::value<int>(&verbosity)->default_value(3), "Set verbosity level, 0 quiet, 1 error, 2 warning, 3 full info (default).")
		("threads,t", po::value<int>(&threads)->default_value(0), "Specifies the maximal number of threads available. Use 0 to determine it automatically.")
		("config,c", po::value<std::string>(&config_file)->default_value(cfg_default), "A path (absolute or relative regarding the program executable) to the configuration file.")
		("log-file,l", po::value<std::string>(&log_file)->default_value(log_default), "A path (absolute or relative regarding current directory) to the log file.")
		("suffix,s", po::value<std::string>(&file_suffix)->default_value("_cleaned"), "A string which will be appended to all output files. Make sure it can really form a valid file name!")
		("destination,d", po::value<std::string>(&destination_path)->default_value("."), "A path to the directory where all output files will be stored. If '.' is specified, output files are saved alongside input files instead of saving into the current working directory.");
...
int main(int argc, const char* argv[]) {
	...
	try {
		...
		if (parse_result != SUCCESS) {
			if (parse_result == EXIT) return SUCCESS;
